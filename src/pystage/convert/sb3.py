        ...
        if "variable" in monitor:
            ...
            if monitor["style"] == "large":
                res += textwrap.dedent(f'''\
                    ...
                    ''')
            # handle list functions in monitor
            #if "mode" == "list" in monitor: (would like to make this cleaner)
            if monitor["style"] == "list":
                res += textwrap.dedent(f'''\
                    ...
                    {stage_var}.{get_translated_function("data_deleteoflist", language)}("{monitor["opcode"]}")
                    {stage_var}.{get_translated_function("pystage_setmonitorposition", language)}("{monitor["opcode"]}", {-240 + monitor["x"]}, {180 - monitor["y"]})
                    {stage_var}.{get_translated_function("data_deletealloflist", language)}("{monitor["opcode"]}")
                    {stage_var}.{get_translated_function("pystage_setmonitorposition", language)}("{monitor["opcode"]}", {-240 + monitor["x"]}, {180 - monitor["y"]})
                ''')
        else:
            # handle builtin variable like timer, answer, xposition and lists
            res += textwrap.dedent(f'''\
